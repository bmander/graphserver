...
class Graph(CShadow):
    ...
    def shortest_path_tree_retro(
        ...
        if walk_options is None:
            walk_options = WalkOptions()
            ret = self._cshortest_path_tree_retro(
                self.soul,
                fromv,
                tov,
                finalstate.soul,
...
class State(CShadow):
    ...
    prev_edge = cproperty(
        lgs.stateGetPrevEdge, c_void_p, EdgePayload, setter=lgs.stateSetPrevEdge
    )

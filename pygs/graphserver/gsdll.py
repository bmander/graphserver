...
declarations = [
    ...
    (
        lgs.epWalk,
        LGSTypes.State,
        [LGSTypes.EdgePayload, LGSTypes.State, LGSTypes.WalkOptions],
    ),
    (
        lgs.epWalkBack,
        LGSTypes.State,
    ...
    (
        lgs.gAddEdge,
        LGSTypes.Edge,
        [LGSTypes.Graph, c_char_p, c_char_p, LGSTypes.EdgePayload],
    ),
    (lgs.gVertices, POINTER(LGSTypes.Vertex), [LGSTypes.Graph, POINTER(c_long)]),
    (
        lgs.gShortestPathTree,
        LGSTypes.ShortestPathTree,
        [
            LGSTypes.Graph,
            c_char_p,
            c_char_p,
            LGSTypes.State,
            LGSTypes.WalkOptions,
            c_long,
            c_int,
            c_long,
        ],
    ...
    (
        lgs.sptAddVertex,
        LGSTypes.SPTVertex,
        [LGSTypes.ShortestPathTree, LGSTypes.Vertex, c_int],
    ),
    ...
    (
        lgs.sptAddEdge,
        LGSTypes.Edge,
        [LGSTypes.ShortestPathTree, c_char_p, c_char_p, LGSTypes.EdgePayload],
    ),
    (
        lgs.sptVertices,
        POINTER(LGSTypes.SPTVertex),
        [LGSTypes.ShortestPathTree, POINTER(c_long)],
    ),
    ...
    (
        lgs.vLink,
        LGSTypes.Edge,
        [LGSTypes.Vertex, LGSTypes.Vertex, LGSTypes.EdgePayload],
    ),
    (
        lgs.vSetParent,
        LGSTypes.Edge,
    ...
    (
        lgs.sptvLink,
        LGSTypes.Edge,
        [LGSTypes.SPTVertex, LGSTypes.SPTVertex, LGSTypes.EdgePayload],
    ),
    (
        lgs.sptvSetParent,
        LGSTypes.Edge,
    ...
    (
        lgs.eWalkBack,
        LGSTypes.State,
        [LGSTypes.Edge, LGSTypes.State, LGSTypes.WalkOptions],
    ),
    ...
    (
        lgs.scPeriodOfOrBefore,
        LGSTypes.ServicePeriod,
        [LGSTypes.ServiceCalendar, c_long],
    ),
    ...
    (
        lgs.spNew,
        LGSTypes.ServicePeriod,
        [c_long, c_long, c_int, POINTER(LGSTypes.ServiceId)],
    ),
    ...
    (
        lgs.spServiceIds,
        POINTER(LGSTypes.ServiceId),
        [LGSTypes.ServicePeriod, POINTER(c_int)],
    ),
    ...
    (
        lgs.cpWalk,
        LGSTypes.State,
        [LGSTypes.CustomPayload, LGSTypes.State, LGSTypes.WalkOptions],
    ),
    (
        lgs.cpWalkBack,
        LGSTypes.State,
    ...
    (
        lgs.elapse_time_and_service_period_forward,
        None,
        [LGSTypes.State, LGSTypes.State, c_long],
    ),
    (
        lgs.elapse_time_and_service_period_backward,
        None,
        [LGSTypes.State, LGSTypes.State, c_long],
    ),
    ...
    (
        lgs.alNew,
        LGSTypes.TripAlight,
        [LGSTypes.ServiceId, LGSTypes.ServiceCalendar, LGSTypes.Timezone, c_int],
    ),
...
def cproperty(cfunc, restype, ptrclass=None, setter=None):
    ...
    return property(
        caccessor(cfunc, restype, ptrclass), cmutator(setter, restype, ptrclass)
    )
...
class SafeWrapper(object):
    ...
    def __getattr__(self, attr):
        ...
        if not getattr(v, "safe", False):
            raise Exception(
                "Using unsafe method %s - you must declare the ctypes restype and argtypes in gsdll.py to ensure 64-bit compatibility."
                % attr
            )
    ...
    def __call__(self, *args):
        ...
        sys.stderr.write(
            ">%s %s(%s)\n"
            % (
                self.lib.restype and self.lib.restype.__name__ or None,
                self.name,
                ",".join(map(repr, args)),
            )
        )

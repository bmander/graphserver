...
#    n_trips = len(stop_time_bundles[0])
#    
#    
...
def gdb_load_gtfsdb_to_boardalight(gdb, agency_namespace, gtfsdb, agency_id, cursor, reporter=sys.stdout):
    ...
    for i, bundle in enumerate(bundles):
        if reporter and i%((n_bundles//n_bundles)+1)==0: reporter.write( "%d/%d trip bundles loaded\n"%(i, n_bundles) )
...
if __name__ == '__main__':
    ...
    gdb_load_gtfsdb_to_boardalight(gdb, options.namespace, gtfsdb, agency_id, gdb.get_cursor())
    gdb.commit()

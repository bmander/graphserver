...
def main():
    ...
    for count, m, t, w, th, f, s, su, start_date, end_date in duped_periods:
        ...
        service_ids = [
            ...
            for x in list(
                gtfsdb.execute(
                    "SELECT service_id FROM calendar where monday=? and tuesday=? and wednesday=? and thursday=? and friday=? and saturday=? and sunday=? and start_date=? and end_date=?",
                    (m, t, w, th, f, s, su, start_date, end_date),
                )
            )
        ]
        ...
        for service_id in service_ids:
            ...
            exception_set_grouper[exception_set] = exception_set_grouper.get(
                exception_set, []
            )
        ...
        for i, exception_set_group in enumerate(exception_set_grouper.values()):
            equivilants.append(
                (
                    "%d%d%d%d%d%d%d-%s-%s-%d"
                    % (m, t, w, th, f, s, su, start_date, end_date, i),
                    exception_set_group,
                )
            )
    ...
    for new_name, old_names in equivilants:
        for old_name in old_names:
            ...
            c.execute(
                "UPDATE calendar SET service_id=? WHERE service_id=?",
                (new_name, old_name),
            )
            c.execute(
                "UPDATE calendar_dates SET service_id=? WHERE service_id=?",
                (new_name, old_name),
            )
            c.execute(
                "UPDATE trips SET service_id=? WHERE service_id=?", (new_name, old_name)
            )

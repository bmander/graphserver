...
def service_calendar_from_timezone(gtfsdb, timezone_name):
    ...
    sc_count = list(
        gtfsdb.execute(
            "SELECT DISTINCT count(*) FROM (SELECT service_id FROM calendar_dates UNION SELECT service_id FROM calendar )"
        )
    )[0][0]
    if sc_count > MAX_CALENDAR_SIZE:
        raise Exception(
            "Service period count %d is greater than the maximum of %d"
            % (sc_count, MAX_CALENDAR_SIZE)
        )
    ...
    for currdate in iter_dates(start_date, end_date):
        ...
        service_period_ends = timezone.normalize(
            currdate_local_start + timedelta(hours=24)
        )

        # enter as entry in service calendar
        cal.add_period(
            TimeHelpers.datetime_to_unix(service_period_begins),

...
class DescribeCrossingAtAlightEvent:
    ...
    def __call__(self, vertex1, edge, vertex2, context):
        ...
        what = "Ride trip %s from stop_seq %s to stop_seq %s" % (
            trip_id,
            vertex1.state.stop_sequence,
            alighting_stop_sequences,
        )
        where = None
        when = None
        geom = self.gtfsdb.shape_between(
...
class AlightEvent:
    ...
    @staticmethod
    def applies_to(vertex1, edge, vertex2):
        return edge is not None and isinstance(
            edge.payload, graphserver.core.TripAlight
        )
...
class HeadwayBoardEvent:
    ...
    def applies_to(vertex1, edge, vertex2):
        return edge is not None and isinstance(
            edge.payload, graphserver.core.HeadwayBoard
        )
...
class HeadwayAlightEvent:
    ...
    def applies_to(vertex1, edge, vertex2):
        return edge is not None and isinstance(
            edge.payload, graphserver.core.HeadwayAlight
        )
...
class StreetStartEvent:
    ...
    def applies_to(edge1, vertex, edge2):
        ...
        return (
            edge1 is None or not isinstance(edge1.payload, graphserver.core.Street)
        ) and (edge2 and isinstance(edge2.payload, graphserver.core.Street))
    ...
    def __call__(self, edge1, vertex, edge2, context):
        ...
        osm_node_id, osm_node_tags, osm_node_lat, osm_node_lon, osm_node_refcount = (
            self.osmdb.node(osm_id)
        )
        ...
        when = "about %s" % str(
            TimeHelpers.unix_to_localtime(vertex.state.time, self.timezone_name)
        )
...
class StreetEndEvent:
    ...
    def applies_to(edge1, vertex, edge2):
        # if edge1 is not a street and edge2 is
        return (
            edge2 is None or not isinstance(edge2.payload, graphserver.core.Street)
        ) and (edge1 and isinstance(edge1.payload, graphserver.core.Street))
    ...
    def __call__(self, edge1, vertex, edge2, context):
        ...
        average_speed = (
            context["sumlength"] / (vertex.state.time - context["lastturntime"])
            if vertex.state.time - context["lastturntime"] > 0
            else 100000000
        )
...
class StreetTurnEvent:
    ...
    def applies_to(edge1, vertex, edge2):
        return (
            edge1
            and edge2
            and isinstance(edge1.payload, graphserver.core.Street)
            and isinstance(edge2.payload, graphserver.core.Street)

...
class DeleteHighwayTypesFilter(OSMDBFilter):
    def run(self, db, *types):
        ...
        for i in range(0, len(purge_list), 100):
            query = "DELETE from ways WHERE id in ('%s')" % "','".join(
                purge_list[i : i + 100]
            )
            c.execute(query)
        db.conn.commit()
        c.close()
        print(
            "Deleted all %s highway types (%s ways)"
            % (", ".join(types), len(purge_list))
        )
...
class DeleteOrphanNodesFilter(OSMDBFilter):
    def run(self, db, *args):
        ...
        c = db.cursor()
        for i in range(0, len(purge_list), 100):
            query = "DELETE from nodes WHERE id in ('%s')" % "','".join(
                purge_list[i : i + 100]
            )
            c.execute(query)
        db.conn.commit()
...
class PurgeDisjunctGraphsFilter(OSMDBFilter):
    def filter(self, db, threshold=None):
        ...
        if not threshold:
            largest = next(
                db.execute(
                    "SELECT graph_num, count(*) as cnt FROM graph_nodes GROUP BY graph_num ORDER BY cnt desc"
                )
            )[0]

            for x in db.execute(
                "SELECT node_id FROM graph_nodes where graph_num != ?", (largest,)
            ):
                node_ids[x[0]] = 1
        else:
            for x in db.execute(
                """SELECT node_id FROM graph_nodes where graph_num in
                                (SELECT a.graph_num FROM 
                                  (SELECT graph_num, count(*) as cnt FROM graph_nodes GROUP BY graph_num HAVING cnt < %s) as a)"""
                % threshold
            ):
                node_ids[x[0]] = 1
...
class FindDisjunctGraphsFilter(OSMDBFilter):
    ...
    def visualize(self, db, out_filename, renderer="/usr/local/bin/prender/renderer"):
        ...
        mr.stroke(255, 0, 0)
        for ct, lat, lon in db.execute(
            "SELECT count(*) as cnt, lat, lon from nodes GROUP BY lat, lon HAVING cnt > 1"
        ):
            if ct > 1:
                mr.point(lon, lat)
...
def main():
    ...
    if len(argv) < 4:
        print(
            "%s <Filter Name> <run|rerun|visualize> <osmdb_file> [<filter args> ...]"
            % argv[0]
        )
        print("Filters:")

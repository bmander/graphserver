...
class OSMGraph(Graph):
    def __init__(self, filename_osmobject_or_stream, projection):
        ...
        for way in osm.ways.values():
            if self.is_valid_way(way):
                ...
                self.add_edge(
                    str(way.fromv), str(way.tov), self.create_edgepayload(way)
                )
                self.add_edge(
                    str(way.tov), str(way.fromv), self.create_edgepayload(way)
                )
        t1 = time.time()
    ...
    def write_graph(
        self, fp, format="%(from)s:%(to)s:%(points)s\n", reproject=True, point_delim=","
    ):
        for edge in self.edges:
            ...
            fp.write(
                format
                % {
                    "from": edge.from_v.label,
                    "to": edge.to_v.label,
                    "name": osmway.tags.get("name", ""),
                    "length": edge.payload.length,
                    "points": point_delim.join(
                        [" ".join([str(c) for c in p]) for p in points]
                    ),
                }
            )
    ...
    def write_spt(
        ...
        fp,
        spt,
        format="%(from)s:%(to)s:%(length)f:%(weight)d:%(points)s\n",
        reproject=True,
        point_delim=",",
    ):

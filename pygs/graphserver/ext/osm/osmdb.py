...
class Way:
    ...
    def __repr__(self):
        return "<Way id='%s' n_nds=%d n_tags=%d>" % (
            self.id,
            len(self.nd_ids),
            len(self.tags),
        )
...
class OSMDB:
    ...
    def set_endnode_ref_counts(self):
        ...
        for i, (node_id, ref_count) in enumerate(endnode_ref_counts.items()):
            ...
            if ref_count > 1:
                c.execute(
                    "UPDATE nodes SET endnode_refs = ? WHERE id=?", (ref_count, node_id)
                )
        ...
        for i, way in enumerate(self.ways()):
            try:
                ...
                for nd in way.nds[1:-1]:  # for every internal node of the way
                    ...
                    if (
                        self.node(nd)[4] > 1
                    ):  # node reference count is greater than one, node is shared by two ways
    ...
    def nearest_node(self, lat, lon, range=0.005):
        ...
        dists = [
            (nid, nlat, nlon, ((nlat - lat) ** 2 + (nlon - lon) ** 2) ** 0.5)
    ...
    def nearest_of(self, lat, lon, nodes):
        ...
        c.execute(
            "SELECT id, lat, lon FROM nodes WHERE id IN (%s)"
...
def osm_to_osmdb(osm_filenames, osmdb_filename, tolerant=False, skipload=False):
    ...
    for osm_filename in osm_filenames:
        if not skipload:
            osmdb.populate(
                osm_filename,
                accept=lambda tags: "highway" in tags,
                reporter=sys.stdout,

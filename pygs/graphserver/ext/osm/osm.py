...
def download_osm(left, bottom, right, top):
    ...
    fp = urlopen(
        "http://api.openstreetmap.org/api/0.5/map?bbox=%f,%f,%f,%f"
...
def dist_haversine(x0, y0, x1, y1):
    ...
    dLat = lat1 - lat0  # rad
    dLon = lon1 - lon0  # rad
    a = sin(dLat / 2) * sin(dLat / 2) + cos(lat0) * cos(lat1) * sin(dLon / 2) * sin(
        dLon / 2
    )
...
class Way:
    ...
    def to_canonical(self, srid, reprojection_func=None):
        ...
        return "SRID=%d;LINESTRING(%s)" % (
            srid,
            ",".join(["%f %f" % (x, y) for x, y in self.get_projected_points()]),
        )

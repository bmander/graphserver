...
class OSMLoadable:
    def load_osm(
        self, osm_filename_or_object, projection, multipliers=[], prefix="osm"
    ):
        ...
        for wayid, way in osm.ways.items():
            if "highway" in way.tags:
                ...
                self.add_edge(
                    prefix + str(way.fromv), prefix + str(way.tov), Street(wayid, len)
                )
                self.add_edge(
                    prefix + str(way.tov), prefix + str(way.fromv), Street(wayid, len)
                )

...
class GraphCrawler(Servable):
    ...
    def vertices(self, like=None):
        if like:
            return "\n".join(
                [
                    '<a href="/vertex?label=&quot;%s&quot;">%s</a><br>' % (vl[0], vl[0])
                    for vl in self.graphdb.execute(
    ...
    def vertex(self, label, currtime=None, hill_reluctance=1.5, walking_speed=0.85):
        ...
        for i, (vertex1, vertex2, edgetype) in enumerate(
            self.graphdb.all_incoming(label)
        ):
            ...
            if s0:
                toterm = (
                    '<a href="/vertex?label=&quot;%s&quot;&currtime=%d">%s@%d</a>'
        ...
        for i, (vertex1, vertex2, edgetype) in enumerate(
            self.graphdb.all_outgoing(label)
        ):

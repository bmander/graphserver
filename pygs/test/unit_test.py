...
class TestStreet:
    ...
    def test_walk_override(self):
        def walk(street_soul, state_soul):
            street = Street.from_pointer(street_soul)
            state = State.from_pointer(state_soul).clone()
            print "walkin!", street, state
            state.time = int(state.time + street.length)
            return state.soul

        def walkback(street_soul, state_soul):
            street = Street.from_pointer(street_soul)
            state = State.from_pointer(state_soul).clone()
            print "walkin back!", street, state
            state.time = int(state.time - street.length)
            return state.soul
        
        Street.setAlgorithms(walk, walkback)
                
        s = Street("longstreet", 2)
        
        state = s.walk(State(0,0))
        
        assert state.time == 2, state
        state = s.walk_back(State(0,10))
        assert state.time == 8, state
        assert state.weight == 0
        assert state.dist_walked == 0
        assert state.prev_edge_type == s.type
        assert state.prev_edge_name == "longstreet"
        assert state.numcalendars == 0

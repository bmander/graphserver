...
from graphserver.core import Timezone, TimezonePeriod
...
class TestTimezone(unittest.TestCase):
    ...
    def test_generate(self):
        ...
        assert (
            tz.utc_offset(1219863600) == -7 * 3600
        )  # august 27, 2008, noon America/Los_Angeles
        assert (
            tz.utc_offset(1199217600) == -8 * 3600
        )  # january 1, 2008, noon America/Los_Angeles
    ...
    def test_time_since_midnight(self):
        ...
        tz = Timezone()
        summer_tzp = TimezonePeriod(
            util.TimeHelpers.localtime_to_unix(
                2008, 6, 1, 0, 0, 0, "America/Los_Angeles"
            ),
            util.TimeHelpers.localtime_to_unix(
                2008, 9, 1, 0, 0, 0, "America/Los_Angeles"
            ),
            -7 * 3600,
        )
        ...
        assert (
            tz.time_since_midnight(
                util.TimeHelpers.localtime_to_unix(
                    2008, 7, 1, 0, 0, 0, "America/Los_Angeles"
                )
            )
            == 0
        )
        assert (
            tz.time_since_midnight(
                util.TimeHelpers.localtime_to_unix(
                    2008, 7, 2, 2, 0, 0, "America/Los_Angeles"
                )
            )
            == 3600 * 2
        )
        ...
        tz = Timezone()
        winter_tzp = TimezonePeriod(
            util.TimeHelpers.localtime_to_unix(
                2008, 1, 1, 0, 0, 0, "America/Los_Angeles"
            ),
            util.TimeHelpers.localtime_to_unix(
                2008, 4, 1, 0, 0, 0, "America/Los_Angeles"
            ),
            -8 * 3600,
        )
        ...
        assert (
            tz.time_since_midnight(
                util.TimeHelpers.localtime_to_unix(
                    2008, 2, 1, 0, 0, 0, "America/Los_Angeles"
                )
            )
            == 0
        )
        assert (
            tz.time_since_midnight(
                util.TimeHelpers.localtime_to_unix(
                    2008, 2, 2, 2, 0, 0, "America/Los_Angeles"
                )
            )
            == 3600 * 2
        )

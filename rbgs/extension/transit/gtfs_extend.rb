...
class Graphserver
  ...
  def load_service_ids
    #initialize sid_numbers as a hash, each service_id will be assigned a number starting from zero
    sid_numbers = {}
  ...
  # Returns a hash with all triphops, processing stop times and frequencies
  def load_triphops
...
--             t1.stop_sequence,
  ...
  def load_google_transit_feed agency_id=nil
    ...
    triphops.each_pair do |sched_key, sched|
      ...
      @gg.add_edge_geom( GTFS_PREFIX+from_id, GTFS_PREFIX+to_id, TripHopSchedule.new( service_id, sched, calendar, tz_offset, 0 ) ,geom)
    end
...
#      @gg.add_vertex( GTFS_PREFIX+from_id ) #Not necessary, that vertex already exists!
      ...
      #In KML Points have the spaces and the comas swapped with respect to postgis
      #We just substitute a space for a comma and viceversa
      geom1.gsub!(/[ ,()A-Z]/) {|s| if (s==' ') then ',' else if (s==',') then ' ' end end}
      ...
      nearby_stops.each do |to_id, dist, geom2|
        #In KML Points have the spaces and the comas swapped with respect to postgis
        #We just substitute a space for a comma and viceversa
        geom2.gsub!(/[ ,()A-Z]/) {|s| if (s==' ') then ',' else if (s==',') then ' ' end end}
  ...
  #Returns a hash of n_stops rows, each row being a hash with keys named label, lat, lon, dist

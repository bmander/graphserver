...
class Vertex
  def to_xml
    ...
    #If the Vertex has a payload, transform it to xml
...
class Graphserver
  ...
  def initialize
    ...
    @server.mount_proc( "/shortest_path" ) do |request, response|
      ...
      #This block checks input parameters and calls for the right shortest path algorithm
    ...
    @server.mount_proc( "/walk_edges" ) do |request, response|
      ...
      begin
        unless vertex = @gg.get_vertex( request.query['label'] ) then raise ArgumentError end
        #Gets the 'time' parameter from the GET request
        init_state = parse_init_state( request )

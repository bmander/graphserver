...
class TestEdgeCachingBug:
    ...
    def test_cache_invalidation_enables_new_edge_discovery(self) -> None:
        ...
        provider.add_edges(
            "B", [(vertex_c, Edge(cost=1.0)), (vertex_d, Edge(cost=2.0))]
        )
        ...
        assert result_ad is not None, (
            "Route A->D should succeed with cache invalidation"
        )
    ...
    def test_cache_invalidation_statistics(self) -> None:
        ...
        assert result_ad is not None, (
            "Route A->D should be found with cache invalidation"
        )
        assert len(result_ad) == 2  # A->B, B->D
        assert result_ad[1].target["id"] == "D"
        ...
        print(
            f"Cache misses: {stats_after_first.cache_misses} -> {final_stats.cache_misses}"
        )

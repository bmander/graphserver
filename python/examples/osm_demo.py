...
def main():
    ...
    print(f"✅ Loaded {provider.node_count:,} nodes, {provider.way_count:,} ways, {provider.edge_count:,} edges")
    ...
    if nearest:
        ...
        # Show any tags
        tags = {k: v for k, v in nearest.items() if k not in {'osm_node_id', 'lat', 'lon'}}
        if tags:
            print(f"   Tags: {tags}")
    ...
    if nearest:
        ...
        print(f"Found {len(node_edges)} connections from node {nearest['osm_node_id']}:")
        for i, (target, edge) in enumerate(node_edges[:3]):  # Show first 3
            ...
            print(f"     Way ID: {way_id}, Distance: {distance:.1f}m, Cost: {edge.cost:.1f}s")
    ...
    for highway, count in sorted(highway_stats.items(), key=lambda x: x[1], reverse=True):
    ...
    print(f"   Spatial index: {len(provider.spatial_index):,} indexed nodes" if provider.spatial_index else "   No spatial index")

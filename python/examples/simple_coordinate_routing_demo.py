...
def main():
    ...
    try:
        ...
        print(f"✅ Loaded: {network_provider.node_count} nodes, {network_provider.edge_count} edges")
        ...
        if start_onramps:
            ...
            print(f"   → Node {target['osm_node_id']}, distance: {edge.metadata['distance_m']:.1f}m")
        ...
        print(f"   Stored target coordinates: {len(access_provider._target_coordinates)}")
        ...
        if offramps_from_2:
            ...
            print(f"   → Coordinate ({target['lat']}, {target['lon']}), distance: {edge.metadata['distance_m']:.1f}m")

        # Test 4: Complete coordinate-to-coordinate routing
        print("\n🛣️  Complete Coordinate-to-Coordinate Routing")
        print("-" * 45)
        
        print(f"Route: ({start_coords['lat']}, {start_coords['lon']}) → ({goal_coords['lat']}, {goal_coords['lon']})")
        ...
        try:
            ...
            if len(result) > 0:
                ...
                for i, edge in enumerate(result):
                    ...
                    if "osm_node_id" in target:
                        print(f"     {i+1}. → OSM Node {target['osm_node_id']} (cost: {cost:.2f}s)")
                    else:
                        print(f"     {i+1}. → Coordinate ({target['lat']}, {target['lon']}) (cost: {cost:.2f}s)")
            ...
            print("   This may be expected if C extension pathfinding is not fully implemented")

...
def download_osm_data(lat_min: float, lon_min: float, lat_max: float, lon_max: float, output_file: str) -> None:
...
def main() -> None:
    ...
    if len(sys.argv) == 6:
        ...
        print("Usage: python download_osm_data.py [lat_min] [lon_min] [lat_max] [lon_max] [output_file]")
        print()
    ...
    print(f"  python -c \"from graphserver.providers.osm import OSMProvider; p = OSMProvider('{output_file}'); print(f'Loaded {{p.node_count}} nodes, {{p.way_count}} ways')\"")
